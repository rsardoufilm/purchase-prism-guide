...
/**
 * Roda varredura no histórico e registra pares similares ainda não vistos.
 * Retorna a lista atual de sugestões pendentes.
 */
export async function scanDuplicates(userId: string): Promise<DuplicateSuggestion[]> {
  ...
  return (pending ?? []) as DuplicateSuggestion[];
}
