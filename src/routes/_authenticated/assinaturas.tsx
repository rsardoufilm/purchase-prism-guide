...
function Assinaturas() {
  ...
  const save = async () => {
    ...
    setOpen(false); setName(""); setAmount(""); setDue("");
    load();
  };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            <Field label="Nome"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Netflix Premium" /></Field>
            ...
            <Field label="Próximo vencimento"><Input type="date" value={due} onChange={(e) => setDue(e.target.value)} /></Field>
          ...
          {rows.map((r) => (
            <div key={r.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 bg-card border border-border rounded-2xl p-4">
              <div className="min-w-0">
                ...
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{r.frequency}{r.next_due_date ? ` • venc. ${new Date(r.next_due_date).toLocaleDateString("pt-BR")}` : ""}</p>
              </div>
